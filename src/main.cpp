...
private:
	...
	// get next swapchain image, or recreate it
	uint32_t imageIndex;
	...
	// reset the wait fence, as we know we are drawing this frame
	...
	// create all the unique queues that we need for our features
	...
	// set up shader stage object
	VkPipelineShaderStageCreateInfo shaderStages[] = {vertShaderStageInfo, fragShaderStageInfo};

	// vertex input state (no bindings and attributes for now)
	...
	// input assembly state for triangles
	...
	// set up viewport state (dynamic state)
	...
	// simple color blending attachment for now
	...
	stbi_uc* pixels = stbi_load("images/people.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	// we need buffers for every frame that could be rendering
	...
	fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // create fence signaled so that first frame doesn't wait

	// create semapohres and fences
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
	...
	// bind vertex buffer (Improvement - could potentially do buffer aliasing)
	...
	// bind descriptor set (uniforms) (QUESTION - if I update the uniform data, and this command buffer isn't re-recorded every frame, does this still work?)
	...
	// Improvement - we should use VulkanMemoryAllocator to have a custom allocator
	...
	// just wait for the copy
	vkQueueWaitIdle(graphicsQueue);

	// delete temporary command buffers
...
private:
    ...
    // pipeline (Improvement - what is the point of saving the layout objects?)
    ...
    // lifetime not managed by myself
